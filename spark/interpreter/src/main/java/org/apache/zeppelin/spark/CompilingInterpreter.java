...
/**
 * Allows to append custom code to spark job. Uses %dep interpreter,
 * so must be called before %spark is initialized.
 * Compiles code written by user, packs it into.jar
 * and adds the jar to %dep interpreter.
 *
...
public class CompilingInterpreter extends Interpreter {
    ...
    public Scheduler getScheduler() {
        ...
        throw new RuntimeException(new InterpreterException("Can't find SparkInterpreter"));
    }
    ...
    public InterpreterResult interpret(String code, InterpreterContext context) {
        ...
        try {
            ...
            return new InterpreterResult(InterpreterResult.Code.ERROR,
                    "Cannot create temporary dir for compiled files");
        }

        StringCompiler compiler =
                new StringCompiler(outputFolder, StringCompiler.currentClassPath());
    ...
    @Override
    public List<InterpreterCompletion>
    completion(String buf, int cursor,
               InterpreterContext interpreterContext) throws InterpreterException {
        return depInterpreter.completion(buf, cursor, interpreterContext);
    }
